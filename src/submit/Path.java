...
public class Path {
    ...
    /** Returns a path from parameter start to a node containing gold with shortest distance from
     * start **/
    public static List<Node> pathToNearestGold(Node start, Node end, ScramState state,
        int stepsLeft) {

//        Node nodeMinPathStartToGold= null; // the node which
        List<Node> minPathStartToGold= null;

//      Invariant pt 1: nodeMinPathStartToGold is n if n
//      i) contains gold.
//      ii) wgtSum(minPathStartToGold) < wgtSum(path(start,n)) + wgt(path((n,end)) < stepsLeft;
//      nodeMinPathStartToGold is null if there are no n satisfying i) and ii)

        // Invariant pt 2: minPathStartToGold is the shortest path from node start to
        // nodeMinPathStartToGold

        for (Node n : state.allNodes()) {

            if (n.getTile().gold() > 0) { // if node has gold and the
                                          // gold not already picked
                List<Node> pathStartToGold= shortest(start, n);
                int wgtpathStartToGold= pathSum(pathStartToGold); // declared for efficiency
                if (minPathStartToGold == null ||
                    wgtpathStartToGold < pathSum(minPathStartToGold)) {

                    List<Node> pathGoldToEnd= shortest(n, end);
                    if (wgtpathStartToGold + pathSum(pathGoldToEnd) < stepsLeft) {

                        minPathStartToGold= pathStartToGold;
                    }
                }
            }
        }

        return minPathStartToGold;
    }
