 ...
 * The CavernPanel should contain only static images that don't need to change <br>
 * unless the screen is redrawn. */
public class CavernPanel extends JPanel {
	...
	private final BufferedImage path;   // Image representing an area the finder can walk on
	...
	/** Create a new CavernPanel of a given size
	 *
	 ...
	 * @param gui          The GUI that owns this CavernPanel */
	public CavernPanel(Cavern cav, int screenWidth, int screenHeight, GUI gui) {
		cavern= cav;
			...
			for (int col= 0; col < cavern.getColumnCount(); col++ ) {
				...
				if (cavern.getTileAt(row, col).getType() == Tile.Type.WALL) {
					...
					page.drawImage(path, TILE_WIDTH * col, TILE_HEIGHT * row,
						TILE_WIDTH, TILE_HEIGHT, null);
					// Darken this tile if the finder has not visited it yet
					if (!visited[row][col]) {
					...
					if (cavern.getTileAt(row, col).gold() > 0) {
